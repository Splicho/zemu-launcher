...
/**
 * trigger-nightly.cjs — fires the nightly build workflow via the `gh` CLI.
 *
 * Usage:
 *   pnpm nightly:trigger
 *   node scripts/trigger-nightly.cjs
 *
 * What it does:
 *   1. Verifies `gh` is installed and authenticated.
 *   2. Runs `gh workflow run nightly.yml --ref main` to dispatch.
 *   3. Polls `gh run list --workflow=nightly.yml` for the run URL
 ...
 *
 * The nightly workflow is configured for `schedule` + `workflow_dispatch`,
 * so this script deliberately does NOT touch git — it's a pure dispatch
 * trigger, the same way `gh workflow run` would be invoked by hand.
 */
...
function dispatchWorkflow() {
  ...
  run('gh', ['workflow', 'run', 'nightly.yml', '--ref', 'main'])
}
...
async function main() {
  console.log(paint('cyan', '\n  ZEmu nightly trigger\n'))

  ensureGhInstalled()
  ensureGhAuthenticated()

  console.log('  Dispatching nightly workflow...')
  dispatchWorkflow()
  ...
  if (!run) {
    ...
    console.error(
      paint('dim', '  Check `gh run list --workflow=nightly.yml --limit 1` to see it.'),
    )
    process.exit(0)
  }
  ...
  console.log('')
  if (run.displayTitle) {
    console.log(paint('green', `  Nightly run triggered: ${run.displayTitle}`))
    console.log('')
  }
