 ...
 *
 * The hook + context object live in `use-license.ts` so this file is
 * a pure component module — Vite's React Fast Refresh can hot-reload
 * it without warning about mixed exports.
 */

import { useEffect, useRef, type ReactNode } from 'react'

import { useAuthContext } from '@/contexts/auth-context'
import { useLicense, LicenseContext } from '@/hooks/use-license'

export function LicenseProvider({ children }: { children: ReactNode }) {
  const { status: authStatus } = useAuthContext()
  // Only run the license hook when we're authed — the
  // `/v1/licenses/validate` endpoint is unauthenticated, but we don't
  // want to surface "License required" to a user who hasn't signed
  // in yet (they're on the login page and the modal would be
  // unreachable anyway).
  ...
  // after the introspect resolves. If the previous render was
  // `'authed'` and now it's anything else, that's a genuine logout —
  // the user explicitly chose to sign out (or their session was
  // rejected) and we should clear the disk record. Anything else
  // (initial mount, the `loading` window, a non-authed page tab)
  // leaves the cached record alone so the next sign-in picks it up.
  // Without this guard the loading window would call `reset()` and
  // delete `license-store.json` before `useLicense`'s mount effect
  // could read it back.
