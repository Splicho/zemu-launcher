  ...
  return (
    <div className="flex h-full flex-col items-start justify-end px-8 pb-8">
      <img
        ...
        alt={t('appSidebar.gameName')}
        className="h-32 object-contain"
      />
      <div className="mt-4 flex flex-col items-star w-full">
        <div className="flex w-full items-center justify-between">
          <h1 className="text-2xl font-bold">{t('appSidebar.gameName')}</h1>
        </div>
        <p className="mt-2 max-w-xl text-sm text-muted-foreground">
          {t('play.gameDescription')}
        </p>
        <div className="mt-6 flex items-center gap-6">
          <GameActionButton />
          <div className="flex items-center gap-2 text-muted-foreground">
            <Box className="size-5" />
            <span className="text-sm">{t('play.gameSize')}</span>
          </div>
